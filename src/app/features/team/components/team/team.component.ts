            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 hover:shadow-lg transition-shadow">
              <div class="flex justify-between items-start mb-4">
                ...
                <div class="flex gap-2">
                  ...
                  <button (click)="deleteTeam(team)" class="p-1.5 text-gray-400 hover:text-red-600 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export class TeamComponent implements OnInit {
  ...
  saveTeam(): void {
    if (this.editingTeam()) {
      this.api.update('teams', this.editingTeam()!._id, this.formData).subscribe({
        next: () => {
          this.loadTeams(1);
          this.closeDialog();
        },
      ...
      this.api.create('teams', this.formData).subscribe({
        next: () => {
          this.loadTeams(1);
          this.closeDialog();
        },
  ...
  deleteTeam(team: Team): void {
    if (confirm('Delete this team?')) {
      this.api.delete('teams', team._id).subscribe({
